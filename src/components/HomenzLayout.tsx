...
export default function HomenzLayout({ children, title }: HomenzLayoutProps) {
  ...
  // Estado para leads novos (substitui tRPC newLeadsQuery)
  const [newLeadsData, setNewLeadsData] = useState<{ count: number; newLeads: unknown[] }>({ count: 0, newLeads: [] });
  const newLeadsQuery = {
    data: newLeadsData,
    refetch: async () => {
      if (!user) return;
      const { data } = await supabase
        .from("leads")
        .select("id, name, lead_score, priority")
        .eq("assigned_seller_id", user.id)
        .gt("created_at", localStorage.getItem(LAST_SEEN_KEY) ?? new Date(Date.now() - 5 * 60 * 1000).toISOString())
        .order("created_at", { ascending: false })
        .limit(10);
      setNewLeadsData({ count: data?.length ?? 0, newLeads: data ?? [] });
    },
  };

  // Polling a cada 30s para buscar leads novos (somente para vendedores) via Supabase
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      newLeads.forEach((lead) => {
        if (!notifiedIds.current.has(String(lead.id))) {
          notifiedIds.current.add(String(lead.id));
          const heatEmoji = lead.priority === "urgent" ? "🔥" : lead.priority === "high" ? "🌡" : "🧊";
