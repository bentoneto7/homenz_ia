...
export default function CalendarTab() {
  ...
  const { data: appointments = [], isLoading, refetch } = trpc.homenz.getAppointments.useQuery(
    { startDate, endDate },
    { refetchInterval: 60000 }
  );

  const updateStatus = trpc.homenz.updateAppointmentStatus.useMutation({
              ...
              <div className="grid grid-cols-2 gap-2">
                {(["confirmed", "completed", "cancelled", "no_show"] as const).map((s) => {
                  ...
                  return (
                    <button
                      key={s}
                      onClick={() => updateStatus.mutate({ appointmentId: selectedAppt.id, status: s })}
                      disabled={updateStatus.isPending || selectedAppt.status === s}
