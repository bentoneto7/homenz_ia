...
function InviteGenerator({ role }: { role: AccessRole }) {
  ...
  const createInvite = {
    isPending: createInvitePending,
    mutate: async (payload: Record<string, unknown>) => {
      setCreateInvitePending(true);
      try {
        const token = crypto.randomUUID();
        const expiresAt = new Date(Date.now() + (payload.expiresInHours as number) * 3600000).toISOString();
        const { error } = await supabase.from("invites").insert({ token, role: payload.role, label: payload.label || null, max_uses: payload.maxUses, expires_at: expiresAt, used: false });
        if (error) throw error;
        setGeneratedUrl(`${window.location.origin}/join?token=${token}`);
      } catch (err: unknown) {
        toast.error(err instanceof Error ? err.message : "Erro ao criar convite");
      } finally { setCreateInvitePending(false); }
    },
  };
...
export default function CreatorPanel() {
  ...
  const claimOwner = {
    isPending: claimOwnerPending,
    mutate: async () => {
      setClaimOwnerPending(true);
      try {
        const { data: { user: authUser } } = await supabase.auth.getUser();
        if (!authUser) throw new Error("Não autenticado");
        await supabase.from("user_roles").upsert({ user_id: authUser.id, role: "owner" });
        toast.success("Você agora é o owner do sistema! Recarregue a página.");
        setTimeout(() => window.location.reload(), 1500);
      } catch (err: unknown) {
        toast.error(err instanceof Error ? err.message : "Erro ao reivindicar owner");
      } finally { setClaimOwnerPending(false); }
    },
  };
  ...
  useEffect(() => { if ((user as Record<string, unknown>)?.role === "owner") refetchInvites(); }, [user]);

  const [allUsers, setAllUsers] = useState<unknown[]>([]);
  const refetchUsers = async () => {
    const { data } = await supabase.from("user_roles").select("*, auth_users:user_id(email)").order("created_at", { ascending: false });
    setAllUsers(data ?? []);
  };
  useEffect(() => { if ((user as Record<string, unknown>)?.role === "owner") refetchUsers(); }, [user]);
  ...
  const setUserRole = {
    mutate: async ({ userId, role }: { userId: string; role: string }) => {
