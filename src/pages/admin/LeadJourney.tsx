...
export default function LeadJourney() {
  ...
  const refetchTimeline = async () => {
    if (!leadId) return;
    const { data } = await supabase.from("lead_events").select("*").eq("lead_id", leadId).order("created_at", { ascending: false });
    setTimeline(data ?? []);
  };
  ...
  useEffect(() => {
    if (!leadId) return;
    setLoadingLead(true);
    supabase.from("leads").select("*").eq("id", leadId).maybeSingle().then(({ data }) => { setLead(data); setLoadingLead(false); });
    refetchTimeline();
    supabase.from("followups").select("*").eq("lead_id", leadId).order("scheduled_at").then(({ data }) => setFollowups(data ?? []));
    const t1 = setInterval(refetchTimeline, 15000);
    const t2 = setInterval(() => {
      supabase.from("leads").select("*").eq("id", leadId).maybeSingle().then(({ data }) => setLead(data));
    }, 30000);
  ...
  const cancelFollowupsMutation = {
    isPending: false,
    mutate: async () => {
      await supabase.from("followups").update({ status: "cancelled" }).eq("lead_id", leadId).eq("status", "pending");
      toast.success("Follow-ups cancelados");
    },
  };
