...
export default function AdminNotifications() {
  ...
  const markRead = { mutate: async ({ id }: { id: string }) => { await supabase.from("notifications").update({ read: true }).eq("id", id); refetch(); }, isPending: false };
  const markAllReadMutation = { mutate: async () => { await supabase.from("notifications").update({ read: true }).eq("read", false); refetch(); }, isPending: false };
