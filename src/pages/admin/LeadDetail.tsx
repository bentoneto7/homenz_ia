...
export default function AdminLeadDetail() {
  ...
  useEffect(() => { if (!id) return; setIsLoading(true); supabase.from("leads").select("*").eq("id", id).maybeSingle().then(({ data }) => { setLead(data); setIsLoading(false); }); }, [id]);
  const [photos, setPhotos] = useState<unknown[]>([]);
  useEffect(() => { if (!id) return; supabase.from("lead_photos").select("*").eq("lead_id", id).then(({ data }) => setPhotos(data ?? [])); }, [id]);
  const [aiResult, setAiResult] = useState<Record<string, unknown> | null>(null);
  useEffect(() => { if (!id) return; supabase.from("ai_results").select("*").eq("lead_id", id).maybeSingle().then(({ data }) => setAiResult(data)); }, [id]);
