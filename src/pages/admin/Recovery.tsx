...
export default function Recovery() {
  ...
  const refetch = async () => {
    setIsLoading(true);
    const { data } = await supabase.from("leads").select("*").in("status", ["cold", "no_show", "lost"]).order("created_at", { ascending: false });
    setLeads(data ?? []);
    setIsLoading(false);
  };
  useEffect(() => { if (isAuthenticated) refetch(); }, [isAuthenticated]);

  const scheduleFollowupMutation = { mutate: async (payload: Record<string, unknown>) => { await supabase.from("followups").insert(payload); refetch(); toast.success("Follow-up agendado!"); }, isPending: false };
