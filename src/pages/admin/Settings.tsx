...
export default function AdminSettings() {
  ...
  const update = { mutate: async (payload: Record<string, unknown>) => { if (!clinic?.id) return; const { error } = await supabase.from("clinics").update(payload).eq("id", clinic.id); if (error) toast.error(error.message); else { toast.success("Configurações salvas!"); } }, isPending: false };;
