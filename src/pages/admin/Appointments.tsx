...
export default function AdminAppointments() {
  ...
  const refetch = async () => {
    if (!clinic) return;
    const query = supabase.from("appointments").select("*, leads(name, phone)").eq("clinic_id", (clinic as Record<string, unknown>).id).order("scheduled_at", { ascending: false }).limit(100);
    if (statusFilter) query.eq("status", statusFilter);
    const { data } = await query;
    setAppointments(data ?? []);
  };
  useEffect(() => { refetch(); }, [clinic, statusFilter]);

  const updateStatus = { mutate: async ({ id, status }: { id: string; status: string }) => { await supabase.from("appointments").update({ status }).eq("id", id); toast.success("Status atualizado!"); refetch(); }, isPending: false };
