...
export default function NetworkDashboardSupabase() {
  ...
  const fetchStats = useCallback(async () => {
    ...
    const res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/network-stats`, {
  ...
  const statsQuery = { isLoading: statsLoading, data: statsData };

  const createInviteMutation = {
    isPending: invitePending,
    mutate: async ({ franchiseId, expiresInDays }: { franchiseId: string; expiresInDays: number }) => {
      setInvitePending(true);
      try {
        const { data: { session } } = await supabase.auth.getSession();
        const res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/create-invite`, {
          method: "POST",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${session?.access_token ?? ""}` },
          body: JSON.stringify({ franchiseId, expiresInDays }),
        });
