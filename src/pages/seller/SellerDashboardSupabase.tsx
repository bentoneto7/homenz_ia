...
export default function SellerDashboardSupabase() {
  ...
  const fetchDash = useCallback(async () => {
    ...
    const { data } = await supabase
      .from("leads")
      .select("id, name, phone, email, hair_loss_type, lead_score, priority, status, created_at, franchise_id, notes")
      .eq("assigned_seller_id", user.id)
      .order("created_at", { ascending: false });
    setDashData({ leads: data ?? [] });
    setDashLoading(false);
  }, [isSeller, user]);

  useEffect(() => { fetchDash(); const t = setInterval(fetchDash, 30000); return () => clearInterval(t); }, [fetchDash]);

  const dashQuery = { isLoading: dashLoading, data: dashData, refetch: fetchDash };

  const handleContact = useCallback(async (leadId: string, franchiseId: string, type: "whatsapp" | "call") => {
    await supabase.from("lead_events").insert({
      lead_id: leadId,
      franchise_id: franchiseId,
      event_type: type === "whatsapp" ? "whatsapp_sent" : "call_made",
