...
export default function CadastroFranqueado() {
  ...
  const loginMutation = {
    ...
    mutate: async ({ email, password }: { email: string; password: string }) => {
      ...
      try {
        ...
        onLoginError(err instanceof Error ? err : new Error("Erro ao fazer login"));
  ...
  const onLoginError = (err: Error) => toast.error(err.message);
  ...
  const registerFranchisee = {
    ...
    mutate: async (payload: Record<string, unknown>) => {
      ...
      try {
        const { email, password, name, whatsapp, instagram, franchiseName, city, state, address, servicePhone } = payload as Record<string, string>;
        const { data: authData, error: signUpError } = await supabase.auth.signUp({
          email, password,
          options: { data: { name, role: "franchisee" } }
        });
        if (signUpError) throw signUpError;
        if (!authData.user) throw new Error("Usuário não criado");
        await supabase.from("franchises").insert({
          owner_id: authData.user.id, name: franchiseName, email, phone: whatsapp,
          instagram: instagram || null, city, state, address: address || null,
          service_phone: servicePhone || null, active: false
        });
        loginMutation.mutate({ email, password });
      } catch (err: unknown) {
        toast.error("Erro ao criar conta", { description: err instanceof Error ? err.message : "Erro desconhecido" });
