...
export default function ClinicLanding() {
  ...
  useEffect(() => {
    if (!slug) return;
    setIsLoading(true);
    supabase.from("landing_pages").select("*, franchises(*)").eq("slug", slug).eq("active", true).maybeSingle()
      .then(({ data, error: sbError }) => {
        if (sbError || !data) setError(new Error("Clínica não encontrada"));
        else setClinic(data);
        setIsLoading(false);
      });
  }, [slug]);
    ...
    mutate: async (payload: Record<string, unknown>) => {
      ...
      try {
        ...
        const { error: sbError } = await supabase.from("leads").insert({
          ...
          landing_page_id: (clinic as Record<string, unknown>)?.id,
        ...
        setSubmitted(true);
        setTimeout(() => navigate(`/c/${slug}/chat/${sessionToken}`), 1500);
      } catch (err: unknown) {
