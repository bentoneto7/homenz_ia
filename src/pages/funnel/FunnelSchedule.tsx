...
export default function FunnelSchedule() {
  ...
  const [slotsData, setSlotsData] = useState<Record<string, unknown> | null>(null);
  const [loadingSlots, setLoadingSlots] = useState(false);
  useEffect(() => {
    if (!clinic) return;
    const franchise = (clinic as Record<string, unknown>)?.franchises as Record<string, unknown>;
    if (!franchise?.id) return;
    setLoadingSlots(true);
    supabase.from("availability_slots").select("*").eq("franchise_id", franchise.id)
      .then(({ data }) => { setSlotsData({ slots: data ?? [] }); setLoadingSlots(false); });
  }, [clinic]);
  ...
  const createAppointment = {
    ...
    mutate: async (payload: Record<string, unknown>) => {
      const { sessionToken, ...rest } = payload as { sessionToken: string; [key: string]: unknown };
      const { data: lead } = await supabase.from("leads").select("id, franchise_id").eq("session_token", sessionToken).maybeSingle();
      if (!lead) { onAppointmentError(new Error("Lead não encontrado")); return; }
      const { error } = await supabase.from("appointments").insert({ ...rest, lead_id: lead.id, franchise_id: lead.franchise_id });
      if (error) { onAppointmentError(error); return; }
      await supabase.from("leads").update({ funnel_step: "scheduled", status: "scheduled" }).eq("session_token", sessionToken);
      onAppointmentSuccess({ sessionToken });
    },
  };;
