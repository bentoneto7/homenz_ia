...
export default function FunnelPhotos() {
  ...
  const uploadPhoto = {
    isPending: false,
    mutate: async ({ sessionToken, photoType, base64 }: { sessionToken: string; photoType: string; base64: string }) => {
      const blob = await fetch(base64).then(r => r.blob());
      const path = `leads/${sessionToken}/${photoType}_${Date.now()}.jpg`;
      await supabase.storage.from("lead-photos").upload(path, blob, { contentType: "image/jpeg", upsert: true });
      const { data: { publicUrl } } = supabase.storage.from("lead-photos").getPublicUrl(path);
      await supabase.from("lead_photos").insert({ session_token: sessionToken, photo_type: photoType, url: publicUrl });
    },
  };
  const updateStep = {
  ...
  const triggerAI = {
    isPending: false,
    mutate: async ({ sessionToken }: { sessionToken: string }) => {
      const { data: { session } } = await supabase.auth.getSession();
      const res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/process-photos`, {
        method: "POST",
      ...
      if (!res.ok) { onAIError(new Error(data.error || "Erro ao processar fotos")); return; }
      onAISuccess(data);
    },
  };
